...
> [!IMPORTANT]
> 当前仓库处于 **v0.1.0-beta** 阶段。软件构建与自动测试已经通过，基本收发已在作者的 STM32F103C6T6 + TJA1050 实物上验证；发布前仍需完成全波特率、长时间和故障恢复验收。它是调试工具，不应用于安全关键控制。
...
- 一个经典高速 CAN 收发器或收发器模块（本项目实测使用 TJA1050）
...
TJA1050 只是本项目作者使用并验证过的型号，不是硬性依赖。其他符合 ISO 11898-2 的经典高速 CAN 收发器/模块通常也能使用，但要逐项确认 MCU 侧逻辑电平、供电电压、待机/使能引脚、最高波特率，以及模块是否已经自带 120 Ω 终端电阻。CAN FD 收发器可以承载经典 CAN 电气信号，但本固件和 STM32F103 的 bxCAN **不支持 CAN FD 帧**。
