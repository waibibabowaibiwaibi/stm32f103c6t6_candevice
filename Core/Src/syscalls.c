...
/*
 * The GNU/Newlib hybrid build needs this ARM EABI thread-pointer stub, while
 * Picolibc already provides a strong definition in libc.  Keep this fallback
 * weak so either runtime can be linked without an undefined or duplicate
 * symbol.
 */
static char __aeabi_tp_area[64];
__attribute__((weak)) void *__aeabi_read_tp(void)
{
  return __aeabi_tp_area;
}
