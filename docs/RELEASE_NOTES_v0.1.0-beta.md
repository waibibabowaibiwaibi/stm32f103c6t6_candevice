...
第一个公开测试版：把 STM32F103C6T6、USB-UART 和 CAN 收发器组合成一个带 Windows / Linux 图形上位机的简易 CAN 调试工具。
...
- 上电默认 500 kbit/s CAN，可在上位机中切换 9 个常用波特率；UART 为 921600 8N1
- Windows / Linux 上位机实时显示、过滤、任务式批量/周期发送和 CSV 导出
- 亮色、暗色和跟随系统主题，可在运行中切换并保存偏好
...
- `UART-CAN-Host-0.1.0-beta-linux-x64.tar.gz`：Linux x64 上位机压缩包
- `UART-CAN-Host-0.1.0-beta-linux-x64.tar.gz.sha256`：Linux 压缩包校验文件
...
- 921600 UART 在文本编码下仍无法保证无损承载高负载 CAN 总线的全部峰值流量
- Windows EXE 尚未代码签名，下载后请核对 SHA-256
- Linux 二进制需要图形桌面及发行版提供的 Qt/XCB 运行库；串口用户需有设备访问权限

## 当前实测环境

- MCU：STM32F103C6T6，8 MHz 外部晶振
- CAN 收发器：SN65HVD230
- USB-UART：PowerWriter 串口设备，921600 8N1
- 下载器：CMSIS-DAP
